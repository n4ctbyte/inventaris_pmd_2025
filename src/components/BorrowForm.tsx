...
const BorrowForm: React.FC<BorrowFormProps> = ({ item, onSuccess, onCancel }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center z-50 p-4 overflow-y-auto animate-fade-in-scale">
      <div className="bg-white/95 backdrop-blur-xl rounded-3xl w-full max-w-md shadow-2xl border border-white/20 my-8 max-h-[90vh] overflow-y-auto animate-fade-in-scale relative">
        {/* Decorative gradient */}
        <div className="absolute inset-0 bg-gradient-to-br from-purple-50/50 to-violet-50/50 rounded-3xl pointer-events-none"></div>
        
        {/* Header */}
        <div className="flex justify-between items-center p-6 border-b border-gray-200/50 relative z-10">
        ...
        {/* Item Info */}
        <div className="p-6 border-b border-gray-100/50 relative z-10">
          <div className="bg-gradient-to-r from-purple-50/80 to-violet-50/80 backdrop-blur-sm rounded-2xl p-5 border border-purple-100/50 shadow-soft">
            <h4 className="font-bold text-gray-900 mb-2 text-lg">{item.name}</h4>
            <p className="text-sm text-gray-600 mb-4 font-medium leading-relaxed">{item.description}</p>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                required
              />
              <p className="text-xs text-gray-500 mt-2 font-medium">Maksimal {item.stock} unit</p>
            </div>
            ...
            <div className="bg-gradient-to-r from-gray-50/80 to-white/80 backdrop-blur-sm rounded-xl p-4 border border-gray-200/50 shadow-soft">
              <div className="flex items-center text-sm text-gray-600">
                <Calendar className="h-4 w-4 mr-2" />
                <span className="font-medium">Tanggal peminjaman: <span className="font-bold text-gray-900">{new Date().toLocaleDateString('id-ID')}</span></span>
              </div>
            </div>
          </div>
